        ...
        if (Input.GetKeyDown(KeyCode.G))//Code de destruction
        {
            foreach(Transform t in Update_grille3d.transform)
            {
                ...
                {                                                                                                                               //ON s'aintéressse en premier lieu à la boite du dessus'
                    if(trouve_boit(this.transform.position + new Vector3(0,1,0)) && trouve_boit(this.transform.position + new Vector3(0,1,0)).libre)//Si trouveBoite rend quelque chose et si ce quelque chose est une boite avec sa variable libre vrai, alors fait sa
                    ...
                    if(trouve_boit(this.transform.position + new Vector3(0,-1,0)))//Si trouveBoite rend quelque chose
                        {
                            if(!trouve_boit(this.transform.position + new Vector3(0,-1,0)).libre)// et si ce quelque chose est une boite avec sa variable libre faus, alors fait sa
                                {
