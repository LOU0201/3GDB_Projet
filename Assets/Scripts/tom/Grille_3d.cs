...
public class Grille_3d : MonoBehaviour
{
    ...
    public void Faire_Trou(Vector3 vec)
    {
        ...
        {
            if (t.transform.position == vec + new Vector3(0, -1, 0))//On s'aint�resse ici, au bloc que l'on veux d�truir c'est � dire celui juste endessous du joueur 
            {
                ...
                if (trouve_boit(t.transform.position + new Vector3(0, -1, 0)))//Si trouveBoite rend quelque chose
                {
                    ...
                    {
                        t.GetComponent<Boite>().Initialisation(true, false, false, false, false);//rend sa variable libre vrai (� la boite en question  (du milieu)), car il y a un blocs compacte en dessous
                        //joueur.GetComponent<Joueur>().ascention(joueur.transform.position);
                    }
                    else
                    {
                        t.GetComponent<Boite>().Initialisation(false, false, false, false, false);//rend sa variable libre fausse (� la boite en question  (du milieu)), car il n'y pas de blocs en dessous'
                    }
                }
            }
        }
